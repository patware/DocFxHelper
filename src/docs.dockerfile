...
# run:   
#   local: docker run -it -d -p 8083:80 -e "NGINX_PORT=80" docs:local
#   local with docfx generated docs: 
#          docker run -it -d -p 8083:80 -e "NGINX_PORT=80" --volume d:\rnd\docker\docfxHelper\virtualVolume\site\:/usr/share/nginx/html/ docs:local 
...
#   Mount ./docs/ /usr/share/nginx/html/
