...
class DocSpecs
{
    ...
    [DocSpec] Add([System.IO.FileInfo]$path)
    {
        ...
        else
        {
            ...
            if ([string]::IsNullOrEmpty($docSpec.Name))
            {
                $docSpec.Name = $docSpec.Id

                if (![string]::IsNullOrEmpty($docSpec.MenuDisplayName))
                {
                    $docSpec.Name = $docSpec.MenuDisplayName
                }
            }
        }
...
function ConvertTo-PowerShellModuleFunctionHelp
{
    ...
    process{
        ...
        Util_Set_MdYamlHeader -file $generated.FullName -key "source" -value $meta
...
{
    param($moduleDetails, $ModuleRelativePath, $CloneUrl)
    ...
    $ret = [ordered]@{
        ...
        RootModuleUri = Get-PowerShellModuleItemUri -CloneUrl $CloneUrl -ModuleRelativePath $ModuleRelativePath -ItemRelativePath $moduleDetails.RootModule
    ...
    switch($spec.Type)
    {
        ...
        DotnetApi {
            ...
            if (Test-Path $docfx_metadata_log_json)
            {
                ...
                $docfx_metadata_log | Group-Object severity
        ...
        PowerShellModule {
            ...
            Import-Module $spec.Name -Force

            $moduleDetails = Get-Module $spec.Name
            $exportedFunctions = Get-PowerShellModuleExportedFunction -ModuleDetails $moduleDetails
            $exportedFunctions | ConvertTo-PowerShellModuleFunctionHelp -CloneUrl $spec.CloneUrl -ModuleRelativePath $spec.SourceRelativePath -Target $Destination
            $viewModel = Get-PowerShellModuleViewModel -moduleDetails $moduleDetails -CloneUrl $spec.CloneUrl $spec.SourceRelativePath
            ...
            $pagesUidPrefix = Get-DocFxHelperResourcePageUidPrefix -Target $Spec.Target
            ConvertTo-DocFxPowerShellModule -Path $Destination -PagesUidPrefix $pagesUidPrefix
